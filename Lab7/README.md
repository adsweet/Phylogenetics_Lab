...
Today we will estimate relationships between gall wasps (Cynipidae) and their close relatives (superfamily Cynipoidea) using Bayes’ theorem and data from four genes (cytochrome oxidase I (COI), elongation factor 1-alpha (EF1a), longwave rhodopsin (LW Rh), and 28S ribosomal RNA (28S)) and morphology. Portions of this lab were adapted directly from Mike Alfaro’s exercise, which he designed for the Workshop in Applied Phylogenetics (2003, in Bodega Bay, CA).
...
Navigate to your mounted volume and create a new directory Lab7

Next, change to the A-State phylogenetics data directory and update the data with:

git pull

Copy the contents of the Lab7_Bayes directory to your Lab7 directory. Change back to your Lab7 directory and extract the contents of this file using:
...
Open Tracer. Click on the "+" sign below the Trace Files window to load your .p file. Make sure you tell the program to look for all types of files, as it defaults to looking for only .log files!
...
What did you choose as a burn-in?

Did your analysis reach stationarity?
...
To run MrBayes commands in non-interactive mode, we simply need to add the commands to the MrBayes block. To modify the Bayes block, download the gall_wasp.nex file and open it in a text editor. Delete the mcmcp nruns=1 line. We set it to only one run so that we would use fewer resources, but it is generally customary to run multiple (2 or more) runs. After the partition command and before the end of the Bayes block, enter the following lines, or copy and paste them in. Note: Because we are running mrBayes non-interactively, we could directly call mcmc instead of calling mcmcp first.
...
This is generally how you’ll want to run MrBayes because saving your commands in a Bayes block makes them repeatable.

After starting your run, detach from your screen by typing CTRL + a, then d. To check on your run, you can reattach to the screen with the command:

$ screen -r mrbayes

When you are out of your screen, you can list the files in your Lab7 directory. Notice there are now two .p and .t files (run1 and run2), because we set the number of runs back to the default in the mcmcp command (nruns=2). Note that MrBayes uses the nexus file name by default for all output files.
...
Did your run reach stationarity?

What burn-in value did you select for this run?

What was the mean likelihood of the post-burn-in data?
...
Save your ASDSF plot as LastName_ASDSF.pdf.

Based on the ASDSF plot, would you update your burn-in?

RWTY?

After you have determined the burn-in, we will look at the parameters that MrBayes estimated for your run using the sump command (summarize the parameters). Note that it is necessary to divide the burn-in from Tracer by the sample frequency chosen in MrBayes (e.g., if Tracer burn-in is 100000 and your samplefreq was 500, you would use burnin=200). 
...
MrBayes > sump burnin=200 filename=gall_wasp.nex;
...
Now that we know our burn-in value and have examined the parameters, we will obtain a consensus tree of your visited/recorded trees. We will use the sumt command like this (to summarize the trees). Note that sumt requires the matrix to be loaded into memory first. The easiest to do this is to simply execute the gall_wasp.nex file, but before doing that, delete the mcmc command. Quit MrBayes, and then edit the file. After getting rid of the mcmc command, you can launch MrBayes again and run the following commands:

MrBayes > execute gall_wasp.nex 
MrBayes > sumt burnin=200 
...
Open gall_wasp.nex.con.tre in FigTree, root on Ibalia, and turn on Branch Labels and select Display: prob. Export the file as LastName_MRC.pdf.
